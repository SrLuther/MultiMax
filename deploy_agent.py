...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("/var/log/deploy-agent.log")],
)
...
def execute_command(command, description, cwd=None, timeout=300):
    ...
    try:
        result = subprocess.run(
            command, cwd=cwd, capture_output=True, text=True, timeout=timeout, check=False  # Não lançar exceção em erro
        )
...
def deploy():
    ...
    if not force:
        try:
            ...
            # Verificar commits
            local_result = execute_command(["git", "rev-parse", "HEAD"], "Obtendo commit local", timeout=5)
            remote_result = execute_command(
                ["git", "rev-parse", "origin/nova-versao-deploy"], "Obtendo commit remoto", timeout=5
            )
    ...
    commands = [
        ...
        (
            ...
            "Rebuild completo do container (sem cache)",
            600,
        ),  # 10 minutos para build
    ...
    try:
        ...
        return jsonify({"ok": False, "error": error_msg, "results": results, "duration": total_duration}), 500
...
def status():
    ...
    if status_info["git_exists"]:
        try:
            result = execute_command(["git", "rev-parse", "HEAD"], "Status: Obtendo commit atual", timeout=5)
            if result["success"]:
