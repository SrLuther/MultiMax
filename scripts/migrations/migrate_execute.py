...
for setor_id, nome in setores:
    try:
        pg_cursor.execute(
            "INSERT INTO setor (id, nome, ativo) VALUES (%s, %s, true) ON CONFLICT DO NOTHING", (setor_id, nome)
        )
        print(f"  [OK] {nome}")
...
for i, nome in enumerate(colaboradores, 1):
    try:
        ...
        sql = (
            "INSERT INTO colaboradores (nome, cpf, ativo, funcao, "
            "departamento, horas_ciclo, saldo_horas, created_at, updated_at) "
            "VALUES (%s, %s, true, %s, %s, %s, %s, NOW(), NOW())"
        )
        pg_cursor.execute(sql, (nome, cpf, "Colaborador", "Operacional", 40.0, 0.0))
        print(f"  [OK] {nome}")
    except Exception as e:
        print(f"  [ERRO] {nome}: {e}")
