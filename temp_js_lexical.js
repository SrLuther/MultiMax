...
const URL_CONFIRMAR_FECHAMENTO = '{{ url_for("ciclos.confirmar_fechamento") }}';
const URL_PDF_GERAL = '{{ url_for("ciclos.pdf_geral") }}';
const URL_RESUMO_FECHAMENTO = '{{ url_for("ciclos.resumo_fechamento") }}';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btnConfirmarFechamento) {
        btnConfirmarFechamento.addEventListener('click', function() {
            if (confirm('Tem certeza que deseja fechar o ciclo? Esta ação não pode ser desfeita.')) {
                ...
                form.method = 'POST';
                form.action = URL_CONFIRMAR_FECHAMENTO;
    ...
    if (btnPDFGeral) {
        btnPDFGeral.addEventListener('click', function() {
            window.open(URL_PDF_GERAL, '_blank');
        });
    }
