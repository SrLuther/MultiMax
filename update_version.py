...
"""
    ...
    content = re.sub(pattern, new_entry.rstrip() + '\n\n' + re.search(pattern, content).group(0) if re.search(pattern, content) else new_entry + content, content, count=1, flags=re.MULTILINE)
...
def main():
    ...
    print(f"4. git push origin nova-versao-deploy")
