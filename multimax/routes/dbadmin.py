                ...
                with urllib.request.urlopen(req, timeout=3) as response:
                    ...
                    if response.status == 200:
                        try:
                            ...
                            health_status = "ok"
                            health_message = data.get(
                                "message", f"Deploy Agent saudável ({health_response_time:.2f}ms)"
                            )
                        except Exception:
                            health_status = "ok"
...
def git_update():
    ...
    if not deploy_agent_url.startswith(("http://127.0.0.1:", "http://localhost:", "http://::1:")):
        ...
        return (
            jsonify(
                {"ok": False, "error": error_msg, "suggestion": "Configure DEPLOY_AGENT_URL para http://127.0.0.1:9000"}
            ),
            400,
        )
    ...
    try:
        ...
        try:
            ...
            current_app.logger.warning(
                f"Erro ao verificar saúde do Deploy Agent: {health_error}. Prosseguindo com deploy..."
            )
        ...
        response = requests.post(
            deploy_endpoint, json=payload, headers=headers, timeout=900  # 15 minutos de timeout (build + down + up)
        )
        ...
        if response.status_code == 200:
            ...
            return jsonify({"ok": False, "error": error_msg, "http_status": response.status_code}), response.status_code
        ...
        return (
            jsonify(
                {"ok": False, "error": error_msg, "suggestion": "Verifique os logs do sistema para mais detalhes."}
            ),
            500,
        )
