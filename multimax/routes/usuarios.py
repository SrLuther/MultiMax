...
def gestao():
    ...
    try:
        ...
        try:
            ...
            if "leave_assignment" in tables:
                try:
                    ...
                    try:
                        db.session.rollback()
                        current_app.logger.warning(f"Database rollback error in balance calculation: {e}")
            ...
            if "hour_bank_entry" in tables:
                try:
                    ...
                    try:
                        db.session.rollback()
                        current_app.logger.warning(f"Database rollback error: {e}")
                    except Exception as rollback_error:
                        current_app.logger.error(f"Failed to rollback database: {rollback_error}")
        except Exception as e:
            try:
        ...
        try:
            db.session.rollback()
            current_app.logger.warning(f"Database rollback error in table migration: {e}")
        except Exception as rollback_error:
            current_app.logger.error(f"Failed to rollback database: {rollback_error}")
    try:
        ...
        try:
            ...
            for c in colaboradores:
                try:
                    ...
                    current_app.logger.warning(f"Error filtering collaborator {c.id}: {e}")
                    continue
        except Exception:
            colaboradores_filtrados = colaboradores
