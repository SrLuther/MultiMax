...
def _premium_footer(canvas, doc, section_name="Documento"):
    ...
    for i in range(16):
        ...
        canvas.setFillColor(
            colors.HexColor(f"#{alpha:02x}{0x17 + int((0x29 - 0x17) * t):02x}{0x2a + int((0x3b - 0x2a) * t):02x}")
        )
        band_h = footer_h / 16
...
def _make_info_block(tipo: str, pairs: list[tuple[str, str]], registrado_por=None):
    ...
    rows: list[list[Any]] = [
        [
            Paragraph("Gerado por", key_style),
            Paragraph(f"{current_user.name} ({current_user.nivel.upper()})", val_style),
        ],
    ...
    try:
        ...
        styles.add(
            ParagraphStyle(
                ...
                fontSize=14,
                leading=18,
                textColor=colors.HexColor(PDF_SECONDARY),
                fontName=_font_bold(),
                spaceBefore=12,
                spaceAfter=8,
            )
        ...
        story.append(
            _make_info_block(
                "Receita",
                [("Nome", r.nome), ("Embalagem", ("Embalado a vacuo" if (r.embalagem or "") == "vacuo" else "Caixa"))],
                registrado_por=None,
            )
        )
...
def exportar_cronograma_pdf():
    ...
    try:
        ...
        data: list[list[Any]] = [
            ["Área/Limpeza", "Frequência", "Última Data", "Próxima Prevista", "Designado(s)", "Observações"]
        ]
        for t in tarefas:
            data.append(
                [
        ...
        for h in historico:
            data_hist.append(
                [
                    ...
                    Paragraph(h.nome_limpeza, styles["Normal"]),
                    h.designados,
                    Paragraph(h.observacao or "-", styles["Normal"]),
                ]
            )
...
def exportar_tarefa_pdf(id):
    ...
    try:
        ...
        story.append(Spacer(1, 0.3 * inch))
        hist = (
            CleaningHistoryModel.query.filter_by(nome_limpeza=tarefa.nome_limpeza)
            .order_by(CleaningHistoryModel.data_conclusao.desc())
            .limit(5)
            .all()
        )
...
def exportar():
    try:
        ...
        product_style = ParagraphStyle(
            name="ProductCell", fontName=_font_normal(), fontSize=9, leading=10, wordWrap="LTR"
        )
        ...
        table = Table(
            data, colWidths=[code_col_width, product_col_width, estoque_in * inch, minimo_in * inch, status_in * inch]
        )
        table.setStyle(_premium_table_style())
        table.setStyle(
            TableStyle(
    ...
    try:
        ...
        story.append(
            _make_info_block(
                ...
                [("Produto", f"{produto.nome}"), ("Código", f"{produto.codigo}")],
                registrado_por=None,
            )
        )
        story.append(Spacer(1, 0.3 * inch))
...
def exportar_relatorio_carnes_pdf(id):
    ...
    try:
        ...
        explain_html = (
            calc_title + "• Peso líquido por parte = Peso bruto − desconto aplicado.<br/>"
...
def exportar_relatorio_carnes_periodo():
    ...
    try:
        ...
        recs = (
            MeatReception.query.filter(MeatReception.tipo == tipo)
        ...
        carriers_all = (
            MeatCarrier.query.filter(MeatCarrier.reception_id.in_(reception_ids)).all() if reception_ids else []
        )
        carriers_by_rec = {}
        for c in carriers_all:
            carriers_by_rec.setdefault(c.reception_id, []).append(c)
        parts_all = (
            MeatPart.query.filter(MeatPart.reception_id.in_(reception_ids)).order_by(MeatPart.id.asc()).all()
            if reception_ids
            else []
        )
        parts_by_rec = {}
        ...
        story.append(
            _make_info_block(
                "Relatório de Recepção de Carnes",
                [
                    ("Escopo", escopo_label),
        ...
        table = Table(
            data_rows,
            colWidths=[date_in * inch, fornecedor_in * inch, ref_in * inch, recebedor_in * inch, total_in * inch],
        )
        table.setStyle(_premium_table_style())
        story.append(table)
        story.append(Spacer(1, 0.3 * inch))
...
def exportar_exemplo_pdf():
    try:
        ...
        story.append(
            _make_info_block(
                "Exemplo de Layout Padrão", [("Seção", "Demonstração"), ("Ambiente", "Local")], registrado_por=None
            )
        )
        story.append(Spacer(1, 0.35 * inch))
    ...
    try:
            ...
            try:
                hq = _range_filter(
                    TimeOffRecord.query.filter(
                        TimeOffRecord.collaborator_id == c.id, TimeOffRecord.record_type == "horas"
                    ),
                    TimeOffRecord.date,
                )
                ...
                cq = _range_filter(
                    TimeOffRecord.query.filter(
                        TimeOffRecord.collaborator_id == c.id,
                ...
                aq = _range_filter(
                    TimeOffRecord.query.filter(
                        TimeOffRecord.collaborator_id == c.id, TimeOffRecord.record_type == "folga_usada"
                    ),
                    TimeOffRecord.date,
                )
                vq = _range_filter(
                    TimeOffRecord.query.filter(
                        TimeOffRecord.collaborator_id == c.id, TimeOffRecord.record_type == "conversao"
                    ),
                    TimeOffRecord.date,
                )
                ...
                for e in hq.order_by(TimeOffRecord.date.desc()).all():
                    eventos.append(
                        ["horas", e.date.strftime("%d/%m/%Y"), c.name, f"{float(e.hours or 0.0):.2f} h", e.notes or ""]
                    )
                for cr in cq.order_by(TimeOffRecord.date.desc()).all():
                    eventos.append(
                        [
                            "dias_add",
                            cr.date.strftime("%d/%m/%Y"),
                            c.name,
                            f"+{int(cr.days or 0)} dia(s)",
                            cr.notes or "",
                        ]
                    )
                for a in aq.order_by(TimeOffRecord.date.desc()).all():
                    eventos.append(
                        ["dias_uso", a.date.strftime("%d/%m/%Y"), c.name, f"-{int(a.days or 0)} dia(s)", a.notes or ""]
                    )
                for v in vq.order_by(TimeOffRecord.date.desc()).all():
                    eventos.append(
                        [
                            ...
                            c.name,
                            f"{int(v.days or 0)} dia(s) — R$ {float(v.amount_paid or 0):.2f}",
                            v.notes or "",
                        ]
                    )
            except Exception:
        ...
        story.append(
            _make_info_block(
                ...
                [
                    (
                        "Colaborador",
                        (next((c.name for c in colaboradores if c.id == collab_id), "Todos") if collab_id else "Todos"),
                    ),
        ...
        for r in resumo:
            data_resumo.append(
                [
                    ...
                    f"{float(r['residual_hours']):.2f} h",
                    f"{int(r['saldo_days'])} dia(s)",
                ]
            )
        ...
        for e in eventos:
            tipo_label = {
                ...
                "dias_add": "Dias acrescentados",
                "dias_uso": "Dias usados",
                "dias_pago": "Dias convertidos em R$",
            }.get(e[0], e[0])
            data_eventos.append(
                [e[1], Paragraph(e[2], styles["Cell"]), tipo_label, e[3], Paragraph(e[4], styles["Cell"])]
            )
        table_events = Table(
            data_eventos, hAlign="LEFT", colWidths=[1.0 * inch, 2.0 * inch, 1.8 * inch, 1.2 * inch, None]
        )
        table_events.setStyle(
            TableStyle(
                [
                    ("GRID", (0, 0), (-1, -1), 0.5, colors.lightgrey),
                    ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#f1f5f9")),
...
class MetricCard(Flowable):
    ...
    def __init__(
        self, title: str, value: str, subtitle: str = "", accent_color: str = PDF_PRIMARY, width: float = None
    ):
        super().__init__()
        self.title = title
        self.value = value
...
def _create_metrics_grid(balance_data: dict, avail_width: float) -> list:
    ...
    metrics = [
        ...
        MetricCard(
            "Dias Convertidos", f"{balance_data['days_from_hours']}", "dias (8h = 1 dia)", PDF_SECONDARY, card_width
        ),
        MetricCard("Horas Residuais", f"{balance_data['residual_hours']:.2f}", "h restantes", PDF_SUCCESS, card_width),
        MetricCard(
            "Saldo de Folgas", f"{balance_data['saldo_days']}", "dias disponíveis", PDF_PRIMARY_DARK, card_width
        ),
...
def exportar_jornada_colaborador_pdf(collaborator_id):
    ...
    try:
        ...
        story.append(
            _make_info_block(
                "Relatório de Jornada — Colaborador",
                [("Colaborador", display_name), ("Período", periodo_str)],
                registrado_por=None,
            )
        )
        story.append(Spacer(1, 0.4 * inch))
        ...
        for r in records:
            ...
            data_table.append(
                [
                    ...
                    dias_str,
                    valor_str,
                    Paragraph(obs_str, styles["Cell"]),
                ]
            )
        table = TableFlowable(
            data_table, colWidths=[1.0 * inch, 1.3 * inch, 0.9 * inch, 0.8 * inch, 1.1 * inch, None], hAlign="LEFT"
        )
...
def view_jornada_pdf():
    ...
    if tipo == "individual" and collaborator_id:
        pdf_url = url_for(
            "exportacao.exportar_jornada_colaborador_pdf", collaborator_id=collaborator_id, inicio=inicio, fim=fim
        )
    else:
        pdf_url = url_for("exportacao.exportar_jornada_todos_pdf", inicio=inicio, fim=fim)

    return render_template(
        "jornada/view_pdf.html", pdf_url=pdf_url, collaborator_id=collaborator_id, inicio=inicio, fim=fim
    )
...
def exportar_jornada_todos_pdf():
    ...
    try:
        ...
        story.append(
            _make_info_block(
                "Relatório de Jornada — Todos os Colaboradores",
                [("Período", periodo_str), ("Total de Colaboradores", str(len(colaboradores_data)))],
                registrado_por=None,
            )
        )
        story.append(Spacer(1, 0.4 * inch))
        ...
        story.append(Spacer(1, 0.2 * inch))
        data_resumo: list[list[Any]] = [
            ["Colaborador", "Total Horas", "Dias Conv.", "Horas Resid.", "Saldo Folgas", "Folgas Usadas", "Conversões"]
        ]
        for cd in colaboradores_data:
            data_resumo.append(
                [
                    Paragraph(cd["display_name"], styles["Cell"]),
                    f"{cd['balance']['total_bruto_hours']:.2f} h",
                    f"{cd['balance']['days_from_hours']}",
                    f"{cd['balance']['residual_hours']:.2f} h",
                    f"{cd['balance']['saldo_days']}",
                    f"{cd['balance']['assigned_sum']}",
                    f"{cd['balance']['converted_sum']}",
                ]
            )
        from reportlab.platypus import Table as TableFlowable
        ...
        for cd in colaboradores_data:
            ...
            table_det = TableFlowable(
                data_table_det,
                colWidths=[1.0 * inch, 1.2 * inch, 0.9 * inch, 0.8 * inch, 1.1 * inch, None],
                hAlign="LEFT",
            )
