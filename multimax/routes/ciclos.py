...
from multimax.services.ciclo_saldo_service import _format_mes_ano, fechar_ciclo_mensal, resumo_em_dias_e_horas
...
# Página: formulário de criação de lote


@bp.route("/lote_horas", methods=["GET", "POST"])
def lote_horas():
    if request.method == "POST":
        # Coleta dados do formulário
        tipo: str | None = request.form.get("tipo")
        horas: str | None = request.form.get("horas")
        observacao: str | None = request.form.get("observacao")
        colaboradores_ids: list[str] = request.form.getlist("colaboradores")
        data_personalizada: str | None = request.form.get("data_personalizada")
        ciclo_semana_id: str | None = request.form.get("ciclo_semana_id")
        ciclo_mes_id: str | None = request.form.get("ciclo_mes_id")
        ...
        if not horas:
            erros.append("Horas são obrigatórias.")
        if not colaboradores_ids:
            erros.append("Selecione ao menos um colaborador.")

        if erros:
            flash(" ".join(erros), "danger")
            return render_template("ciclos/lote_horas.html")

        # Pré-visualização: repassa dados para a tela de preview
        colaboradores = Collaborator.query.filter(Collaborator.id.in_(colaboradores_ids)).all()
        return render_template(
            "ciclos/lote_horas_preview.html",
            tipo=tipo,
            horas=horas,
            observacao=observacao,
            colaboradores=colaboradores,
            data_personalizada=data_personalizada,
            ciclo_semana_id=ciclo_semana_id,
            ciclo_mes_id=ciclo_mes_id,
        )
...
# Página: pré-visualização e confirmação do lote
@bp.route("/lote_horas/preview", methods=["POST"])
def lote_horas_preview():
    # Recebe dados do preview e confirma operação
    if request.method == "POST":
        tipo: str | None = request.form.get("tipo")
        horas: str | None = request.form.get("horas")
        observacao: str | None = request.form.get("observacao")
        ...
        # Cria operação de lote
        bulk = BulkHourOperation()
        bulk.created_by = str(current_user.id)
        bulk.custom_date = data_personalizada
        bulk.hours = float(horas)
        bulk.type = tipo
        bulk.observation = observacao or ""
        bulk.cycle_week_id = int(ciclo_semana_id) if ciclo_semana_id else None
        bulk.cycle_month_id = int(ciclo_mes_id) if ciclo_mes_id else None
        bulk.total_collaborators = len(colaboradores_ids)
        db.session.add(bulk)
        db.session.flush()  # Garante ID para bulk_id
        # Cria registros de TimeOffRecord para cada colaborador
        for cid in colaboradores_ids:
            reg = TimeOffRecord()
            reg.collaborator_id = int(cid)
            reg.date = data_personalizada
            reg.record_type = tipo
            reg.hours = float(horas)
            reg.origin = "lote"
            reg.notes = observacao or ""
            reg.created_by = str(current_user.id)
            reg.bulk_id = bulk.id
            db.session.add(reg)
        db.session.commit()
        flash(f"Lote aplicado para {len(colaboradores_ids)} colaborador(es).", "success")
        return redirect(url_for("ciclos.lote_horas_logs"))

    # Se não for POST, redireciona para o formulário
    return redirect(url_for("ciclos.lote_horas"))


# Página: logs/auditoria de lotes
@bp.route("/lote_horas/logs", methods=["GET"])
def lote_horas_logs():
    # Listar todos os lotes de horas, ordenados do mais recente para o mais antigo
    lotes = BulkHourOperation.query.order_by(BulkHourOperation.created_at.desc()).all()
    return render_template("ciclos/lote_horas_logs.html", lotes=lotes)


# Página: correção de lote
@bp.route("/lote_horas/corrigir/<lote_id>", methods=["GET", "POST"])
def lote_horas_corrigir(lote_id):
    # Buscar lote original
    lote: Any | None = BulkHourOperation.query.get(lote_id)
    if not lote:
        flash("Lote não encontrado.", "danger")
        return redirect(url_for("ciclos.lote_horas_logs"))

    if request.method == "POST":
        # Coleta dados de correção
        tipo: str | None = request.form.get("tipo")
        horas: str | None = request.form.get("horas")
        observacao: str | None = request.form.get("observacao")
        colaboradores_ids: list[str] = request.form.getlist("colaboradores")
        ...
        # Cria novo lote de correção
        lote_correto = BulkHourOperation()
        lote_correto.created_by = str(current_user.id)
        lote_correto.custom_date = data_personalizada
        lote_correto.hours = float(horas)
        lote_correto.type = tipo
        lote_correto.observation = observacao or ""
        lote_correto.cycle_week_id = int(ciclo_semana_id) if ciclo_semana_id else None
        lote_correto.cycle_month_id = int(ciclo_mes_id) if ciclo_mes_id else None
        lote_correto.total_collaborators = len(colaboradores_ids)
        lote_correto.correction_of_id = lote.id
        db.session.add(lote_correto)
        db.session.flush()
        # Cria registros de TimeOffRecord para cada colaborador
        for cid in colaboradores_ids:
            reg = TimeOffRecord()
            ...
            reg.record_type = tipo
            reg.hours = float(horas)
            reg.origin = "lote_correção"
            reg.notes = observacao or ""
            reg.created_by = str(current_user.id)
            reg.bulk_id = lote_correto.id
            db.session.add(reg)
        db.session.commit()
        flash(f"Lote de correção aplicado para {len(colaboradores_ids)} colaborador(es).", "success")
        return redirect(url_for("ciclos.lote_horas_logs"))

    # GET: exibe dados do lote original e colaboradores
    colaboradores: list[Any] = [r.collaborator for r in lote.time_off_records]
    return render_template(
        "ciclos/lote_horas_corrigir.html",
        lote=lote,
        colaboradores=colaboradores,
    )
...
def _calculate_collaborator_balance(collaborator_id):
    ...
    total_horas_decimal: Any | Decimal = _get_active_ciclos_query(collaborator_id).with_entities(
    ...
    if not isinstance(total_horas_decimal, Decimal):
        ...
        total_horas: Decimal = total_horas_decimal
    ...
    if total_horas_float < 0:
        ...
        horas_restantes: float = total_horas_float % 8.0
    ...
    valor_aproximado: Decimal = Decimal(str(dias_completos)) * valor_dia
...
def _calculate_collaborator_balance_range(collaborator_id, start_date, end_date, setor_id=None):
    """Calcula saldo do colaborador dentro de um período, opcionalmente filtrando por setor."""
    q: Query = _get_active_ciclos_query(collaborator_id).filter(
    ...
    total_horas_decimal: Any | Decimal = q.with_entities(
        func.coalesce(func.sum(Ciclo.valor_horas), 0)
    ).scalar() or Decimal("0.0")
...
def _month_start_end(anchor):
    ...
    month_end: date = next_month - timedelta(days=1)
    return month_start, month_end


def _get_open_cycle_anchor_date():
    ...
    try:
        ...
        candidates: list[Any] = [d for d in (min_horas, min_folgas, min_oc) if d]
        if candidates:
...
def _get_open_cycle_current_date():
    ...
    try:
        ...
        candidates: list[Any] = [d for d in (max_horas, max_folgas, max_oc) if d]
        if candidates:
...
def _week_start_sunday(d):
    ...
    days_since_sunday: int = (d.weekday() + 1) % 7
    return d - timedelta(days=days_since_sunday)


def _cycle_label_for_week(week_start, week_end, month_ref):
    """
    ...
    first_sunday: date = month_start + timedelta(days=(6 - month_start.weekday()) % 7)
    ...
    n: int = 1 + int((week_start - first_sunday).days // 7)
    return f"Ciclo {n} | {_month_name_pt(month_ref)}"


def _weekly_cycles_for_open_month(current_date):
    ...
    first_week_start: date = _week_start_sunday(month_start)

    out: list[dict[str, object]] = []
    cur: date = first_week_start
    while cur <= current_date:
        week_start: date = cur
        week_end: date = cur + timedelta(days=6)

        period_start: date = max(week_start, month_start)
        period_end: date = min(week_end, current_date)
        label: str = _cycle_label_for_week(week_start, week_end, current_date.month)
        ...
        cur: date = cur + timedelta(days=7)

    return out
    return out


def _weekly_cycles_for_month(anchor):
    """
    ...
    for week_start, week_end in weeks:
        label: str = _cycle_label_for_week(week_start, week_end, anchor.month)
...
def _infer_reference_month_from_weeks(weeks):
    """Tenta inferir mês de referência (PT) a partir das semanas arquivadas no formato 'Mês Ano'."""
    if not weeks:
        now: date = datetime.now(ZoneInfo("America/Sao_Paulo")).date()
    ...
    best_month: int = max(counts.items(), key=lambda kv: kv[1])[0]
    return _month_name_pt(best_month)


def _calculate_collaborator_balance_for_cycle(collaborator_id, ciclo_id):
    """Saldo por colaborador dentro de um ciclo mensal fechado (somente registros fechados desse ciclo_id)."""
    total_horas_decimal: Any | Decimal = Ciclo.query.filter(
...
def _get_ciclo_atual():
    ...
    anchor: date = _get_open_cycle_anchor_date()
    mes_inicio: str = _month_name_pt(anchor.month)
...
def _validate_hours_format(value_str, allow_negative=False):
    ...
    try:
        ...
        resto: float = abs(float(valor)) % 0.5
...
def index():
    ...
    try:
        current_date: date = _get_open_cycle_current_date()
        month_start, _month_end = _month_start_end(current_date)
        week_start_raw: date = _week_start_sunday(current_date)
        week_end_raw: date = week_start_raw + timedelta(days=6)
        ciclo_semana_atual = {
        ...
        for colab in colaboradores:
            ...
            if selected_setor_id:
                balance: dict[str, float | int] = _calculate_collaborator_balance_range(
        ...
        # Totais gerais
        total_horas_geral: int = sum(s["balance"]["total_horas"] for s in colaboradores_stats)
        total_dias_geral: int = sum(s["balance"]["dias_completos"] for s in colaboradores_stats)
        total_horas_restantes_geral: int = sum(s["balance"]["horas_restantes"] for s in colaboradores_stats)
        total_valor_geral: int = sum(s["balance"]["valor_aproximado"] for s in colaboradores_stats)

        # Verificar se há registros ativos no mês (para mostrar botão de fechamento mensal)
        tem_registros_ativos: bool = (
        ...
        selected_collaborator_id: int | None = request.args.get("collaborator_id", type=int)
        selected_collaborator = None
        ferias = []

        if selected_collaborator_id and current_user.nivel in ("admin", "DEV"):
            selected_collaborator: Any | None = Collaborator.query.get(selected_collaborator_id)
...
def _summary_from_hours(total_horas_float):
    ...
    try:
        ...
        if total_horas_float < 0:
            ...
            hrs_rest: float = total_horas_float % 8.0
...
def _process_week_details(s, ciclo_id, status):
    """Processa detalhes de uma semana (horas, folgas, ocorrências)."""
    week_start: object = s["week_start"]  # type: ignore[assignment]
    week_end: object = s["week_end"]  # type: ignore[assignment]
    ...
    folgas = []  # Corrige erro de sorted sem lista (reportCallIssue)
...
def pesquisa():
    ...
    try:
        ...
        ciclo_id: int | None = request.args.get("ciclo_id", type=int)
        ...
        ciclo_ids: list[int] = _extrair_ciclo_ids(semanas_detalhe)
...
def _buscar_semanas_fechadas(q: str, q_month_name: str | None, ciclo_id: int | None):
    query: Query = CicloSemana.query
    if ciclo_id:
        query: Query = query.filter(CicloSemana.ciclo_id == ciclo_id)
    if q_month_name:
        query: Query = query.filter(CicloSemana.label.ilike(f"%{q_month_name}%"))
    elif q:
        query: Query = query.filter(CicloSemana.label.ilike(f"%{q}%"))
...
def _detalhar_semana_fechada(s):
    ...
    total_horas: float = _total_horas(horas)
...
def _buscar_semanas_ativas(q: str):
    ...
    semanas_open: list[dict[str, object]] = _weekly_cycles_for_open_month(current_date)
...
def _mesclar_folgas_utilizadas(horas, folgas, status: str):
    ...
    for h in horas:
        if getattr(h, "origem", None) != "Folga utilizada":
            continue
        try:
            valor_horas: float = float(h.valor_horas) if h.valor_horas is not None else -8.0
        except (ValueError, TypeError):
            valor_horas: float = -8.0
        extra_folgas.append(
            SimpleNamespace(
...
def _extrair_ciclo_ids(semanas_detalhe: list[dict[str, object]]) -> list[int]:
    ciclo_ids: list[int] = []
    for s in semanas_detalhe:
        cid: object | None = s.get("ciclo_id")
        try:
            if cid is not None:
...
def _datas_fechamento() -> tuple[date, date]:
    anchor_before_close: date = _get_open_cycle_anchor_date()
    _, month_end = _month_start_end(anchor_before_close)
    next_month_start: date = month_end + timedelta(days=1)
...
def _registros_ativos(setor_id: int | None = None):
    query: Query = Ciclo.query.filter(Ciclo.status_ciclo == "ativo")
    if setor_id:
...
def _criar_carryover_e_fechar_registros(colaboradores_totais, next_month_start, proximo_ciclo_id) -> None:
    usuario = current_user.name or current_user.username
    for cid, dados in colaboradores_totais.items():
        colaborador: Any | None = Collaborator.query.get(cid)
...
def _fechar_folgas_e_ocorrencias(proximo_ciclo_id) -> None:
    try:
...
def _arquivar_ciclos_semanais(proximo_ciclo_id, anchor_before_close) -> None:
    try:
        CicloSemana.query.filter(CicloSemana.ciclo_id == proximo_ciclo_id).delete()
        semanas: list[dict[str, object]] = _weekly_cycles_for_month(anchor_before_close)
        for s in semanas:
            cs = CicloSemana()
...
def _registrar_fechamento_e_log(proximo_ciclo_id, totais_gerais, colaboradores_totais=None) -> None:
    ...
    observacoes: str = request.form.get("observacoes", "").strip()
    ...
    if colaboradores_totais:
        mes_ano_atual: str = _format_mes_ano()
        usuario = current_user.name or current_user.username
...
def lancar_horas() -> Any:
    ...
    try:
        ...
        data_lancamento_str: str = request.form.get("data_lancamento", "").strip()
        origem: str = request.form.get("origem", "").strip()
        descricao: str = request.form.get("descricao", "").strip()
        valor_horas_str: str = request.form.get("valor_horas", "").strip()
        ...
        # Validar colaborador
        collaborator: Any | None = Collaborator.query.get(collaborator_id)
        if not collaborator:
            flash("Colaborador não encontrado.", "danger")
            return redirect(url_for("ciclos.index"))

        # Validar origem
        origens_validas: list[str] = ["Domingo", "Feriado", "Horas adicionais", "Outro", "Folga utilizada"]
        ...
        allow_negative: bool = origem == "Folga utilizada"
        ...
        # Validar data
        data_lancamento: date = datetime.strptime(data_lancamento_str, "%Y-%m-%d").date()
...
def folgas_adicionar() -> Any:
    ...
    try:
        ...
        data_str: str = (request.form.get("data_folga") or "").strip()
        tipo: str = (request.form.get("tipo") or "").strip().lower()
        dias_str: str = (request.form.get("dias") or "1").strip()
        obs: str = (request.form.get("observacao") or "").strip()
        ...
        dias: int = int(dias_str) if dias_str else 1
        if dias <= 0:
            dias = 1

        collaborator: Any | None = Collaborator.query.get(cid)
        if not collaborator:
            flash("Colaborador não encontrado.", "warning")
            return redirect(url_for("ciclos.index"))

        data_folga: date = datetime.strptime(data_str, "%Y-%m-%d").date()
...
def ocorrencias_adicionar() -> Any:
    ...
    try:
        cid = int((request.form.get("collaborator_id") or "0").strip() or "0")
        data_str: str = (request.form.get("data_ocorrencia") or "").strip()
        tipo: str = (request.form.get("tipo") or "").strip().lower()
        desc: str = (request.form.get("descricao") or "").strip()
        ...
        data_oc: date = datetime.strptime(data_str, "%Y-%m-%d").date()
        o = CicloOcorrencia()
...
@login_required
def ocorrencias_excluir(id: int) -> Any:
    if current_user.nivel not in ("admin", "DEV"):
        flash("Acesso negado.", "danger")
        return redirect(url_for("ciclos.index"))
    try:
...
@login_required
def historico(collaborator_id) -> Any:
    ...
    try:
        ...
        total_ciclos: int = len(semanas)
        pages: int = max(1, int(math.ceil(total_ciclos / float(per_page)))) if total_ciclos else 1
        if page < 1:
            page = 1
        if page > pages:
            page: int = pages
        ...
        semanas_page: list[dict[str, object]] = semanas[start_idx:end_idx]
        ...
        for s in semanas_page:
            ...
            if week_start and week_end:
                ...
                resumo: dict[str, float | int] = _calculate_collaborator_balance_range(
...
def excluir(ciclo_id) -> Any:
    ...
    try:
        ciclo: Any | None = Ciclo.query.get(ciclo_id)
...
def resumo_fechamento() -> Any:
    ...
    try:
        ...
        for colab in colaboradores:
            ...
            registros_query: Query = _get_active_ciclos_query(colab.id)
            if selected_setor_id:
                # Incluir: registros com setor_id preenchido OU registros antigos (NULL) do colaborador neste setor
                from sqlalchemy import or_

                registros_query: Query = registros_query.filter(
                    or_(
                        Ciclo.setor_id == selected_setor_id,
                        (Ciclo.setor_id.is_(None) & (Collaborator.setor_id == selected_setor_id)),
                    )
            ...
            if total_horas > 0:  # Só incluir se tiver horas
                ...
                saldo_visual: str = resumo_em_dias_e_horas(horas_restantes)
        ...
        # Totais gerais
        total_geral_horas: int = sum(r["total_horas"] for r in colaboradores_resumo)
        total_geral_dias: int = sum(r["dias_completos"] for r in colaboradores_resumo)
        total_geral_horas_restantes: int = sum(r["horas_restantes"] for r in colaboradores_resumo)
        total_geral_valor: int = sum(r["valor"] for r in colaboradores_resumo)
        ...
        mes_ano_proximo: str = _format_mes_ano()
        saldos_info = []
        for resumo in colaboradores_resumo:
            saldos_info.append(
                {
...
def confirmar_fechamento():
    ...
    try:
        # Filtro por setor (opcional)
        selected_setor_id: int | None = request.form.get("setor_id", type=int)
...
def config():
    ...
    if request.method == "POST":
        try:
            novo_nome: str = request.form.get("nome_empresa", "").strip()
            novo_valor_str: str = request.form.get("valor_dia", "").strip()
...
def atestado_adicionar():
    ...
    try:
        ...
        cid_code: str = request.form.get("cid", "").strip()
        medico: str = request.form.get("medico", "").strip()

        if data_fim < data_inicio:
            flash("Data final deve ser maior ou igual à data inicial.", "warning")
            return redirect(url_for("ciclos.index", collaborator_id=cid))

        dias: int = (data_fim - data_inicio).days + 1

        foto_filename = ""
        if "foto_atestado" in request.files:
            foto: FileStorage = request.files["foto_atestado"]
            if foto and foto.filename:
                ALLOWED_IMAGE_EXTENSIONS: set[str] = {"png", "jpg", "jpeg", "gif", "webp", "bmp"}
                ...
                upload_dir: str = os.path.join("static", "uploads", "atestados")
                os.makedirs(upload_dir, exist_ok=True)
                safe_name: str = secure_filename(foto.filename)
                ext: str = safe_name.rsplit(".", 1)[1].lower() if "." in safe_name else "jpg"
                foto_filename: str = f"{datetime.now().strftime('%Y%m%d%H%M%S')}_{cid}.{ext}"
...
def atestado_excluir(id):
    ...
    try:
        if m.foto_atestado:
            path: str = os.path.join("static", "uploads", "atestados", m.foto_atestado)
...
def pdf_individual(collaborator_id):
    ...
    try:
        ...
        ultimo_fechamento = CicloFechamento.query.order_by(CicloFechamento.ciclo_id.desc()).first()
        ciclo_id = (ultimo_fechamento.ciclo_id + 1) if ultimo_fechamento else 1
        mes_inicio: str = _month_name_pt(current_date.month)
        ...
        response: Response = make_response(pdf)
...
@login_required
def pdf_individual_ciclo(collaborator_id, ciclo_id):
    ...
    try:
        ...
        balance: dict[str, float | int] = _calculate_collaborator_balance_for_cycle(collaborator_id, ciclo_id)
        nome_empresa = _get_nome_empresa()
        valor_dia: float = _get_valor_dia()
...
def pdf_geral():
    ...
    try:
        ...
        # Totais gerais
        total_horas_geral: int = sum(r["balance"]["total_horas"] for r in colaboradores_resumo)
        total_dias_geral: int = sum(r["balance"]["dias_completos"] for r in colaboradores_resumo)
        total_horas_restantes_geral: int = sum(r["balance"]["horas_restantes"] for r in colaboradores_resumo)
        total_valor_geral: int = sum(r["balance"]["valor_aproximado"] for r in colaboradores_resumo)
...
def pdf_geral_ciclo(ciclo_id):
    ...
    try:
        ...
        colaboradores_resumo = []
        for colab in colaboradores:
            balance: dict[str, float | int] = _calculate_collaborator_balance_for_cycle(colab.id, ciclo_id)
            semanas_detalhadas = []
            tem_algo = False
            for w in weeks:
                horas = (
...
def setores_editar(
    setor_id,
) -> tuple[Response, Literal[403]] | tuple[Response, Literal[400]] | Response | tuple[Response, Literal[500]]:
...
def setores_toggle(
    ...
    try:
        ...
        status: str = "ativado" if setor.ativo else "desativado"
...
def _gerar_pdf_ciclo_aberto_bytes():
    ...
    base_url: str = str(base_dir) if base_dir else os.getcwd()
    pdf_bytes: bytes | None = HTML(string=html, base_url=base_url).write_pdf()
...
def enviar_pdf_ciclo_aberto() -> (
    tuple[Response, Literal[403]] | tuple[Response, Literal[500]] | tuple[Response, Literal[400]] | Response
):
    """
    ...
    try:
        ...
        if not isinstance(mes_inicio, str):
            ...
            mes_inicio_str: str = mes_inicio
        nome_arquivo_base: str = re.sub(r"[^a-zA-Z0-9_]+", "_", mes_inicio_str.strip())
        nome_arquivo: str = f"Ciclos_{nome_arquivo_base}.pdf"
