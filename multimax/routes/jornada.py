...
def _calculate_collaborator_balance(collaborator_id, date_start=None, date_end=None):
    ...
    # Folgas usadas
    aq = TimeOffRecord.query.filter(*filters, TimeOffRecord.record_type == "folga_usada")
    assigned_sum = int(aq.with_entities(func.coalesce(func.sum(TimeOffRecord.days), 0)).scalar() or 0)

    # Conversões em dinheiro
    vq = TimeOffRecord.query.filter(*filters, TimeOffRecord.record_type == "conversao")
            ...
            if record_type == "horas" and record.hours and record.hours > 0:
                try:
                    ...
                    if missing > 0:
                        ...
                        flash(
                            f"Registro adicionado. Conversão automática: +{missing} dia(s) e -{missing*8}h.", "success"
                        )
                    else:
...
def export():
    ...
    writer.writerow(
        ["Data", "Colaborador/Usuário", "Tipo", "Horas", "Dias", "Valor Pago", "Origem", "Observações", "Criado por"]
    )
...
def arquivar():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            records_to_archive = TimeOffRecord.query.filter(
                TimeOffRecord.date >= period_start, TimeOffRecord.date <= period_end
            ).all()
            ...
            db.session.commit()
            flash(
                f"{archived_count} registro(s) arquivado(s) com sucesso. Todos os dados foram reiniciados.", "success"
            )
            return redirect(url_for("jornada.index"))
    ...
    suggested_end = last_record.date if last_record else date.today()
    return render_template(
        "jornada/arquivar.html", active_page="jornada", suggested_start=suggested_start, suggested_end=suggested_end
    )
...
def historico_colaborador(collaborator_id):
    ...
    archived_records = (
        JornadaArchive.query.filter_by(collaborator_id=collaborator_id).order_by(JornadaArchive.date.desc()).all()
    )
    current_records = (
        TimeOffRecord.query.filter_by(collaborator_id=collaborator_id).order_by(TimeOffRecord.date.desc()).all()
    )
    ...
    total_folgas_creditos = sum(
        r["days"] or 0
        for r in all_records
        if r["record_type"] == "folga_adicional" and (not r.get("origin") or r.get("origin") != "horas")
    )
