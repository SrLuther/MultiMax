...
def _get_all_collaborators():
    """Retorna todos os colaboradores (User e Collaborator são uma coisa só)"""
...
def index():
    ...
    # Buscar férias e atestados (se colaborador selecionado)
    ferias_list = []
    atestados_list = []
    if collaborator_id:
        ferias_list = Vacation.query.filter(
            Vacation.collaborator_id == collaborator_id
        ).order_by(Vacation.data_inicio.desc()).limit(50).all()
        
        atestados_list = MedicalCertificate.query.filter(
            MedicalCertificate.collaborator_id == collaborator_id
        ).order_by(MedicalCertificate.data_inicio.desc()).limit(50).all()
...
def novo():
    ...
    return render_template('jornada/novo.html', colaboradores=colaboradores, active_page='jornada', hoje=hoje)
...
def ferias_adicionar():
    ...
    try:
        ...
        if not collaborator_id or not data_inicio_str or not data_fim_str:
        ...
        v = Vacation()
        v.collaborator_id = collaborator_id
...
def atestado_adicionar():
    """Adicionar registro de atestado médico"""
    if current_user.nivel not in ('admin', 'DEV', 'operador'):
        flash('Acesso negado.', 'danger')
        return redirect(url_for('jornada.index'))
        ...
        if 'foto_atestado' in request.files:
            ...
            if foto and foto.filename:
                if not allowed_file(foto.filename):
                    flash('Tipo de arquivo não permitido. Use imagens (PNG, JPG, JPEG, GIF, WEBP, BMP).', 'warning')
                    return redirect(url_for('jornada.index', collaborator_id=collaborator_id))
                ...
                foto_filename = f"{datetime.now().strftime('%Y%m%d%H%M%S')}_{collaborator_id}.{ext}"
                foto.save(os.path.join(upload_dir, foto_filename))
        
        m = MedicalCertificate()
        m.collaborator_id = collaborator_id
...
def atestado_excluir(id):
    """Excluir registro de atestado médico"""
    if current_user.nivel not in ('admin', 'DEV', 'operador'):
        flash('Acesso negado.', 'danger')
        return redirect(url_for('jornada.index'))
