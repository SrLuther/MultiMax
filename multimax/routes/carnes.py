...
from .. import db
from ..models import MeatCarrier, MeatPart, MeatReception
...
def nova():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            existentes = (
                MeatReception.query.with_entities(MeatReception.reference_code)
                .filter(MeatReception.reference_code.like(f"{pref}%"))
                .all()
            )
            maior = 0
...
def relatorio(id: int):
    ...
    filtered_animais = {
        an: ps
        for an, ps in animais.items()
        if (sum((p.get("peso_bruto") or 0.0) for p in ps) > 0.0)
        or (sum((p.get("peso_liquido") or 0.0) for p in ps) > 0.0)
    }
    ...
    return render_template(
        "carnes_relatorio.html", r=r, carriers=carriers, animais=filtered_animais, totals=totals, active_page="carnes"
    )
