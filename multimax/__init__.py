  ...
  <div class="db-container">
    ...
    def inject_notifications():
        try:
            ...
            if nf != "limpeza":
                low_stock = (
                    Produto.query.with_entities(
                        Produto.id, Produto.nome, Produto.quantidade, Produto.estoque_minimo, Produto.codigo
                    )
            ...
            if nf != "estoque":
                overdue = (
                    CleaningTask.query.with_entities(
                        CleaningTask.id, CleaningTask.nome_limpeza, CleaningTask.proxima_data
                    )
            ...
            reads = (
                NotificationRead.query.with_entities(NotificationRead.tipo, NotificationRead.ref_id)
    ...
    def _get_version():
        ...
        try:
            ...
            if r.returncode == 0 and r.stdout.strip():
                return r.stdout.strip()
            r2 = subprocess.run(
                ["git", "rev-parse", "--short", "HEAD"], cwd=base_dir, capture_output=True, text=True, timeout=2
            )
    ...
    if isinstance(resolved_version, str) and resolved_version:
        ...
        if processed_version:
            ...
            app.config["APP_VERSION_RESOLVED"] = '2.6.53'
    else:
        app.config["APP_VERSION_RESOLVED"] = '2.6.53'
    ...
    @app.context_processor
    def inject_version():
        ver = app.config.get("APP_VERSION_RESOLVED", '2.6.53')
    ...
    with app.app_context():
        try:
            ...
            try:
                ...
                if last_tag:
                    rlog = subprocess.run(
                        ["git", "log", "--pretty=%h %s", f"{last_tag}..HEAD"],
                        cwd=base_dir,
                        capture_output=True,
                        text=True,
                        timeout=3,
        ...
        try:
            ...
            try:
                ...
                if missing_tables:
                    app.logger.info(
                        f"Criando automaticamente {len(missing_tables)} tabela(s) ausente(s): {missing_tables}"
                    )
                    db.create_all()
                    db.session.commit()
        ...
        if not is_sqlite and db_ok:
            try:
                ...
                db.session.execute(
                    text('alter table public."meat_reception" add column if not exists reference_code text')
                )
                role = None
                try:
                    uri_s = str(uri)
                ...
                try:
                    ...
                    db.session.execute(text("create index if not exists idx_produto_nome on produto (nome)"))
                    db.session.execute(
                        text("create index if not exists idx_hist_product_date on historico (product_id, data)")
                    )
                    db.session.execute(
                        text("create index if not exists idx_notif_user_ref on notification_read (user_id, ref_id)")
                    )
                    db.session.execute(
                        text("create index if not exists idx_cleaningtask_proxima on cleaning_task (proxima_data)")
                    )
                    db.session.execute(
                        text("create index if not exists idx_shift_collab_date on shift (collaborator_id, date)")
                    )
                    db.session.execute(text("create index if not exists idx_produto_codigo on produto (codigo)"))
                    db.session.execute(
                        text("create index if not exists idx_produto_quantidade on produto (quantidade)")
                    )
                    db.session.execute(
                        text("create index if not exists idx_produto_minimo on produto (estoque_minimo)")
                    )
                    db.session.execute(text("create index if not exists idx_historico_action on historico (action)"))
                    db.session.commit()
                except Exception:
        ...
        if db_ok:
            ...
            try:
                ...
                if not s_ver:
                    s_ver = AppSetting()
                    s_ver.key = "app_version"
                    db.session.add(s_ver)
        ...
        try:
            ...
            if "time_off_record" not in table_names:
                if is_sqlite:
                    ...
                    db.session.execute(
                        text("CREATE INDEX IF NOT EXISTS idx_timeoff_collab ON time_off_record (collaborator_id)")
                    )
                    db.session.execute(text("CREATE INDEX IF NOT EXISTS idx_timeoff_date ON time_off_record (date)"))
                    db.session.execute(
                        text("CREATE INDEX IF NOT EXISTS idx_timeoff_type ON time_off_record (record_type)")
                    )
                else:
                    db.session.execute(
                        text(
        ...
        def _make_backup(
            retain_count: int = 20, min_interval_sec: int = 900, update_daily_snapshot: bool = True, force: bool = False
        ):
            try:
                    ...
                    if src and os.path.exists(src):
                        ...
                        files_regular = sorted(
                            [
                                p
                                for p in files_all
                                if os.path.basename(p).startswith("backup-")
                                and not os.path.basename(p).startswith("backup-24h")
                            ],
                        ...
                        files_regular = sorted(
                            [
                                ...
                                and not f.startswith("backup-24h")
                            ],
                            key=lambda p: os.path.getmtime(p),
                            reverse=True,
                        )
            ...
            def _loop():
                while True:
                    try:
                        with app.app_context():
                            _make_backup(
                                retain_count=20, min_interval_sec=900, update_daily_snapshot=False, force=False
                            )
                    except Exception:
                        pass
                    time.sleep(900)
                            ...
                            for s in shifts:
                                ...
                                try:
                                    ...
                                    if (s.shift_type or "").lower().find("5h") >= 0:
                                        is_5h = True
                                    if is_5h:
                                        exists_b = TemporaryEntry.query.filter_by(
                                            kind="folga_hour_both", collaborator_id=cid, date=s.date
                                        ).first()
                                        if not exists_b:
                                            tb = TemporaryEntry()
                                        ...
                                        exists = TemporaryEntry.query.filter_by(
                                            kind="folga_credit", collaborator_id=cid, date=s.date
                                        ).first()
                                        if not exists:
                                            te = TemporaryEntry()
