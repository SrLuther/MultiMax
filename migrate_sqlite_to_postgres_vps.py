...
try:
    ...
    for setor_id, nome in setores:
        ...
        if nome not in existing_setores:
            pg_cursor.execute(
                'INSERT INTO public.setor (nome, descricao, ativo) VALUES (%s, %s, %s)',
                (nome, f'Setor: {nome}', True)
            )
            setores_inseridos += 1
            print(f'  [+] {nome}')
        else:
        ...
        if nome and nome.strip():
            try:
                pg_cursor.execute(
                    ...
                    (nome.strip(), f'CPF-{nome.replace(" ", "-")}', 'Colaborador', 'Operacional', 
                     True, 40.0, 0.0, datetime.now(), datetime.now())
                )
